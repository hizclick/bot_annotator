...
if __name__ == '__main__':
    ...
    doubled = [tweets.append(raw_tweet[i])for i in range(len(raw_tweet))]

    amharic_punctuation = ['።','፣','፤','…','፥','”','“','...']

    all_tweets = ' '.join(tweets)
    exclude = set(string.punctuation)
    all_tweets = ''.join(ch for ch in all_tweets if ch not in exclude)
    all_tweets = ''.join(ch for ch in all_tweets if ch not in amharic_punctuation)
